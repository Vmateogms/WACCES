...
if !bannerchoice!==exit (
exit
)

if !bannerchoice!==help (
cls
echo.
...
echo - wifiscan	: Performs a WIFI scan
echo - interface	: Select an interface for attack 
echo - attack	: Attacks on selected WIFI
echo.
echo For More Information, please read "readme.txt".
echo.
echo.
echo Other proyects by VMG:
echo "https://github.com/Vmateogms"
echo.
echo Press any key to continue...
pause >nul
cls
...
if !bannerchoice!==interface (
	cls
	call :deteccion_interfaz
	call :interfaz_seleccionada
	cls
	goto :main
)

if !bannerchoice!==wifiscan (
	del infogate.xml
	call :wifiscan
...
if !bannerchoice!==attack (
set /a attemp=1

	if "!targetwifi!"=="No WIFI Selected" (
call colorchar.exe /0c "Please select a WIFI..."
echo.
echo.
echo  Press any key to continue...
timeout /t 5 >nul
...
cls
echo [==================================================]
echo colorchar.exe /07 "   Target WIFI:"
echo !targetwifi!
call calorchar.exe /07 "   Total passwords:"
call colorchat.exe /0f "!password_number!"
echo.
echo [==================================================]
...
call :find_connection_state
	...
	echo.
	echo [==================================================]
	call colorchar.exe /0c "   Password not found... Try with a different dictionary"
	echo.
	echo [==================================================]
	echo Press any key to continue...
	pause >nul
...
cls
	...
	if "!interface_id"==0(
	echo.
	call colorchar.exe /0c "You have at least '1' WIFI interface to perform a scan..."
	echo.
	echo.
	echo Press any key to continue...
	timeout /t 5 >nul
...
set /a keynumber=0
echo.
call calorchar.exe /0b " Possible WIFI Networks"
echo.
echo.
...
echo.
echo Low Signal Strength WIFIs are not recommended
echo.
for /f "tokens=1-4" %%a in ('netsh wlan show networks mode^=bbssid interface^="interface_id!" ') do (


if %%a==SSID (
	set /a keynumner=!keynumber! + 1
	set current_ssid=%%d
...
set current_signal==%%c
	...
	call colorcher.exe /03 " - !current_signal:~1,5!"
	echo.

	echo !keynumber! - !current_ssid! - !current_signal:~1,5!>>wifilist.txt	
	if !keynumber!==24 ( goto:skip_scan)	
